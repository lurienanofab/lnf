...
namespace LNF.Tests
{
    ...
    public class ModelFactoryTests : TestBase
    {
        ...
        private void AssertResource(IResource r)
        {
            ...
            Assert.AreEqual(0, r.CurrentReservationID);
            Assert.AreEqual(0, r.CurrentClientID);
            Assert.AreEqual(0, r.CurrentActivityID);
            Assert.AreEqual(false, r.CurrentActivityEditable);
            Assert.AreEqual(null, r.CurrentFirstName);
            Assert.AreEqual(null, r.CurrentLastName);
            Assert.AreEqual(null, r.CurrentActivityName);
            Assert.AreEqual(null, r.CurrentBeginDateTime);
            Assert.AreEqual(null, r.CurrentEndDateTime);
            Assert.AreEqual(null, r.CurrentNotes);
        ...
        private void AssertReservation(IReservation rsv)
        {
            ...
            Assert.AreEqual("613280200002161121000022000F018139", rsv.AccountNumber);
            Assert.AreEqual(17, rsv.OrgID);
            Assert.AreEqual("University of Michigan", rsv.OrgName);
            Assert.AreEqual(true, rsv.NNINOrg);
            Assert.AreEqual(true, rsv.PrimaryOrg);
            Assert.AreEqual(1, rsv.OrgTypeID);
            Assert.AreEqual("U of Michigan (US)", rsv.OrgTypeName);
            Assert.AreEqual(5, rsv.ChargeTypeID);
            Assert.AreEqual("UMich", rsv.ChargeTypeName);
