...
namespace LNF.Impl.Billing
{
    public class RoomDataCleanGenerator
    {
        ...
        public RoomDataCleanGenerator(ISession session, DateTime sd, DateTime ed, int clientId = 0, int roomId = 0)
        {
            Session = session;
            StartDate = sd;
            EndDate = ed;
        ...
        public RoomDataClean[] Generate(IEnumerable<Event> events)
        {
            var result = Filter(events);
            return result;
        }

        private IEnumerable<Cost> GetCurrentCosts()
        {
            if (_costs == null)
                _costs = Session.FindCosts(new[] { "RoomCost" }, EndDate);
        ...
        public DataTable EventsToDataTable(IEnumerable<Event> events)
        {
            ...
            var rooms = Session.Query<Room>().Where(x => x.Active).ToList();
        ...
        private RoomDataClean[] Filter(IEnumerable<Event> events)
        {
            ...
            var clients = Session.Query<Client>().Where(x => clientIds.Contains(x.ClientID));

            string[] roomNames = events.Where(x => x.DeviceDescription != null).Select(x => x.DeviceDescription).Distinct().ToArray();
            var rooms = Session.Query<Room>().Where(x => roomNames.Contains(x.RoomName));
